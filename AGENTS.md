...
- `m365/` defines the Microsoft 365 Copilot surface: `m365/manifest.json` (M365 Unified App Manifest v1.28, Dynamic Client Registration), `m365/color.png` / `m365/outline.png` (icons from `assets/logo.svg`), `m365/tools/supabase-tools.json` (`mcpToolDescription`, required by the v1.28 schema), `m365/scripts/generate-tools.mjs` to regenerate that file from the live MCP server, and `m365/scripts/check-package.py` (companion-file + `mcpToolDescription` guard shared by the validate and sync workflows). `.github/workflows/sync-m365-tools.yml` keeps the tool descriptions current (nightly, `secrets.SUPABASE_ACCESS_TOKEN`). See [`m365/README.md`](m365/README.md). Only skills ≤ 20 companion files ship (the MS per-skill cap); the PR/main validation workflow enforces it. The release ships a single `supabase-plugin.tar.gz` that includes `m365/`.
